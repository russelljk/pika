 ...
 *
 * @note Root object are destroyed liked ordinary objects. The only difference is that there is no need
...
public:
    ...
    /** Resume a previously paused collector.
     *  @note The collector will not run. 
     */
    void ResumeNoRun();
...
private:
    ...
    //  The active context. Since we do not perform
    //  a write barrier on each stack push, when we enter into a new scope, etc... ; It becomes
    //  necessary to [re]mark the current Context before the sweep stage can begin.
    ...
    */
    GCObject* blacks; // Seen and marked.
    GCObject* grays;  // Seen but not marked.
    GCObject* whites; // Unmarked and unseen; Potential garbage.
    GCObject* scan;   // Used during the mark phase, can point anywhere between grays and whites.
    GCObject* sweep;  // Used during sweep phase to deallocate all white objects.
    
    RootObject* head; // Circular list of all root objects.
    // TODO: Perhaps a simple array would be easier to use for root objects
};
