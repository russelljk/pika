...
/** Hook interface used by Pika. Hooks are called at certain points during a programs execution
  * and can be used to provide debuggers, tracing, statistics and importing among other things.
  * Registering a Hook is done by calling Engine::AddHook with the HookEvent the hook responds to.
  * Hooks can be registered to multiple events, just beware that release is called once (in order)
  * for every event the hook is registered to (see IHook::Release). This means a Hook registered for
  * HE_return and HE_yield will have Release called twice; First Release(HE_return) then Release(HE_yield).
  */
struct PIKA_API IHook {
    ...
    /** Called in response to an event. 
      ...
      * @result         Returns a bool specifying if the event was handled and the next hook should not be called.
      */
